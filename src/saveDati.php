...
switch ($op) {
    ...
    case 4: //provincia
        ...
        //06/07/2018: aggiunto il controllo se la provincia è già stata inserita, allora non si fa nulla
        $pr=$db->mysqli_real_escape($_POST["provincia"]);
        $esiste=$provinciaClass->esisteProvincia($db, $pr);
        if(mysqli_num_rows($esiste)){
            //non fare nulla
    ...
    case 5: //comune
        ...
        //06/07/2018: aggiunto il controllo se il comune è già stato inserito, allora non si fa nulla
        $cm=$db->mysqli_real_escape($_POST["comune"]);
        $idpr=$db->mysqli_real_escape($_POST["province"]);
        $esiste=$comuneClass->esisteComuneInProvincia($db, $cm, $idpr);
        if(mysqli_num_rows($esiste)){
            //non fare nulla
