...
function getPreviousUOTId($current_id) {
    global $db;
    $conn = $db->conn(); // Usa la funzione conn() esistente
    $query = "SELECT IdUot FROM uot WHERE IdUot < '" . $conn->real_escape_string($current_id) . "' ORDER BY IdUot DESC LIMIT 1";
...
function getNextUOTId($current_id) {
    global $db;
    $conn = $db->conn(); // Usa la funzione conn() esistente
    $query = "SELECT IdUot FROM uot WHERE IdUot > '" . $conn->real_escape_string($current_id) . "' ORDER BY IdUot ASC LIMIT 1";
    $result = $db->query($query);
    if ($row = $db->fetchassoc($result)) {
        return $row['IdUot'];
    }
