<?php // Inizio PHP
	...
	$searchuot = null;
	$searchregione = null;
        $searchruolo = null; // aggiunto con nuova modifica al codice del 02/07/2024 11:05

	if(!isset($_GET['ordinaPer'])) 
		$_GET['ordinaPer'] = 'uot.Denominazione';

	$ordinaPer = $_GET['ordinaPer'];
        if(isset($_POST["searchPersonaleUOT"]) || isset($_POST["searchuot"]) || isset($_POST["searchregione"]) || isset($_POST["searchruolo"])) { // *** INIZIO MODIFICA ***
            ...
            $searchruolo = isset($_POST["searchruolo"]) ? $_POST["searchruolo"] : null; // *** INIZIO MODIFICA ***
        } else {
	...
	while ($r = $db->fetchassoc2($elencoruoli)) {
        ...
        // Aggiunto con nuova modifica al codice del 02/07/2024 11:05 
        // *** INIZIO MODIFICA: Query per dropdowns 02/07/2024 11:05 ***
                    ...
                    <aside>
                        <div class="centered-wrapper">
                            <div class="button-container" style="margin-bottom: 10px;">
                                ...
                                <!-- Aggiunto con nuova modifica al codice del 02/07/2024 11:05 
                                *** INIZIO MODIFICA con i filtri Dropdown 02/07/2024 11:05 *** -->
                                
                                <input name="searchpersonaleuot" type="text" placeholder="Cognome del Verificatore*" value="<?=$searchPersonaleUOT?>">
                                
                                
                                <!-- *** INIZIO MODIFICA: Dropdown per UOT 02/07/2024 11:05 *** -->
                                <select name="searchuot">
                                    <option value="">-- Seleziona una UOT --</option>
                                    <?php while ($row_uot = $db->fetchassoc($result_uot)) { ?>
                                        <option value="<?= $row_uot['IdUot'] ?>" <?= ($searchuot == $row_uot['IdUot']) ? 'selected' : '' ?>>
                                            <?= htmlspecialchars($row_uot['denominazione']) ?>
                                        </option>
                                    <?php } ?>
                                </select>
                                <!-- *** FINE MODIFICA Dropdown per UOT *** -->

                                <!-- *** INIZIO MODIFICA: Dropdown per Regione *** -->
                                <select name="searchregione">
                                    <option value="">-- Seleziona una Regione --</option>
                                    <?php while ($row_regione = $db->fetchassoc($result_regione)) { ?>
                                        <option value="<?= $row_regione['idRegione'] ?>" <?= ($searchregione == $row_regione['idRegione']) ? 'selected' : '' ?>>
                                            <?= htmlspecialchars($row_regione['regione']) ?>
                                        </option>
                                    <?php } ?>
                                </select>
                                <!-- *** FINE MODIFICA Dropdown per Regione *** -->

                                <!-- *** INIZIO MODIFICA: Dropdown per Ruolo (Verificatori non attivi) *** -->
                                <select name="searchruolo">
                                    <option value="">-- Seleziona un Ruolo --</option>
                                    <?php while ($row_ruolo = $db->fetchassoc($result_ruolo)) { ?>
                                        <option value="<?= $row_ruolo['IdRuolo'] ?>" <?= ($searchruolo == $row_ruolo['IdRuolo']) ? 'selected' : '' ?>>
                                            <?= htmlspecialchars($row_ruolo['ruolo']) ?>
                                        </option>
                                    <?php } ?>
                                </select>
                                <!-- *** FINE MODIFICA Dropdown per Ruolo *** -->
                                
                                <!-- *** vecchio codice prima della modfica del 02/07/2024 ***
                                <input name="searchverificatori" type="text" placeholder="Cognome del Verificatore" value="<?=$searchverificatore?>">
                                <input name="searchuot" type="text" placeholder="nome della UOT" value="<?=$searchuot?>">
                                <input name="searchregione" type="text" placeholder="nome della Regione" value="<?=$searchregione?>">
                                -->
                                ...
                                <div class="row spacer"></div> <!-- Riga vuota sopra - aldo -->
                                <span style="font-size: smaller;">* il campo è keysensitive</span>
                                
                                <!-- *** FINE MODIFICA 02/07/2024 11:05 *** -->
                                
                                <!-- *** vecchio codice prima della modfica del 02/07/2024 ***
                                <a href="verificatori_out.php" class="button">Verificatori non attivi</a>
                                -->
