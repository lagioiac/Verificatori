...
include 'db/mysql.php'; /* aggiunto per flg_Admin */
...
if ($item) //se corrette...
{
	/* aggiunto per flg_Admin */
        $db = new DbConnect();
        $db->open() or die($db->error());
    
        $_SESSION['user'] = $email; // Assumendo che $email sia l'utente autenticato
        
        /* set del FLAG per indicare se l'utente loggato è Admin */
        $query = "SELECT * FROM utenti WHERE email = '" . $db->mysqli_real_escape($email) . "' AND password = '" . $db->mysqli_real_escape($password) . "'";
        $result = $db->query($query);   
        $user_flg = $db->fetchassoc($result);
        $_SESSION['flgAdmin'] = $user_flg['flgAdmin'];
